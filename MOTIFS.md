...
| Main page tabs | Freshness green / static blue / stale black — `ui/tab_bar.py`, palette tab tokens. An **unselected** tab that has **not updated** uses a 10% white fill (`_TAB_PAGE_NOT_UPDATED_IDLE`) and a stronger light outline (`_TAB_OUTLINE_STALE`, 1.5px) so it does not vanish into the bar. |
