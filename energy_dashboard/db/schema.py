...
def setup_sqlite_schema(path: str) -> tuple[bool, str]:
    """Create parent dirs, SQLite file, and logger tables."""
    ...
    try:
        ...
        try:
            conn.execute(_GROWATT_DDL)
            conn.execute(_TASMOTA_DDL)
            conn.execute(_TASMOTA_DEVICES_DDL)
            conn.execute(_OCTOPUS_DDL)
            conn.commit()
        finally:
            conn.close()
...
def setup_mysql_schema(host: str, port: int, user: str, password: str, database: str) -> tuple[bool, str]:
    """CREATE DATABASE if needed, then create tables."""
    ...
    try:
        conn = pymysql.connect(
            host=host or "localhost", port=int(port), user=user, password=password,
...
def setup_postgresql_schema(host: str, port: int, user: str, password: str, database: str) -> tuple[bool, str]:
    """Create database if missing (connect via postgres/template1), then CREATE TABLE IF NOT EXISTS."""
    ...
    try:
        conn = psycopg2.connect(
            host=host or "localhost", port=int(port), dbname=database,
    ...
    for adm in ('postgres', 'template1'):
        try:
            admin_conn = psycopg2.connect(
                host=host or "localhost", port=int(port), dbname=adm,
                user=user, password=password,
            )
            break
        except Exception:
