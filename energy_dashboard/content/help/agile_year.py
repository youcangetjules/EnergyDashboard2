...
<p>
<b>Avg −1y / −2y / −3y</b> are the daily <b>average</b> on the <b>same calendar date</b> one, two, and three years earlier (for example 23 Sep 2026 looks up 23 Sep 2025). After the price, a smaller figure in brackets is that year’s average <b>minus this day’s average</b> (negative = cheaper than this year). They come only from days already in this table — a dash means that prior date is not stored yet (keep fetching over time), or that the date cannot exist (29 Feb in a non-leap year). These columns are table-only; the chart is unchanged.</p>
<p>
Toggle <b>Import (household)</b> / <b>Export (outgoing)</b> to switch series. The tariff codes are the ones typed on <b>Forecasts</b>. <b>Fetch year</b> asks Octopus for the public unit-rate history (about three years — enough for <b>Avg −1y</b> / <b>Avg −2y</b> when that tariff has the history). Those half-hour slots go into <code>agile_price_snapshots</code>; the daily high / low / average go into <code>agile_year_daily</code> so the tab can reload from the database and the since-start trend can grow beyond one fetch. On PostgreSQL the owner must run the Setup &amp; Info CREATE script once so that new table exists (the dashboard login is not allowed to create it). If Octopus has no rates before a certain date for that product (or you only ever fetched one year), the table’s first day is that start — not a chart bug.</p>
<p>
