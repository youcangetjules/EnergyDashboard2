...
<p>
Between the connectivity table and the text summary, an <b>animated diagram</b> shows external APIs, LAN devices, this dashboard (&quot;house&quot;), databases, and export. Box borders and link colours track connectivity health. When something is <b>degraded</b> (Grott stale in Hybrid, a live alarm, a failed service), a <b>DEGRADED</b> banner at the top spells out what failed and what is carrying live data (for example &quot;GROTT stale — Hybrid live path is Growatt cloud API&quot;). Affected boxes get WARN badges and plain-English subtitles; click any highlighted box for the full &quot;What&apos;s happening right now&quot; write-up. Where that box has a login (Growatt cloud, Grott, EMQX, Tasmota, the inverter web page, Modbus, PVOutput, or a database), the same window includes those username / password / API-key fields from Setup &amp; Info. <b>Test</b> leaves a connectivity line in the bottom-right of that panel. Green means the last test passed, red means it failed, and amber means the last pass is more than an hour old (<b>Connectivity - last OK (Stale &gt;1hr since last test)</b>). That line stays after you close the window. The Octopus box uses the API key and meter numbers from the Octopus tabs. <b>Save</b> and <b>Test</b> write the same place those screens do. Wonderwatt keeps its share-link field. Drag the horizontal <b>splitter</b> between the table and the diagram to resize the two panes.</p>
<p>
