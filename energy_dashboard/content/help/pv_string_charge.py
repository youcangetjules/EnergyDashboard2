...
<p>
Each string card shows <b>now</b> (live PV power from that string, in kW) and the energy for the day on screen (London midnight): kWh that string generated, plus how much of that is estimated to have gone into the battery. On today that line starts “Today since 00:00”. On an earlier day it starts with that date. If stored samples only start later, the card says so instead of pretending the day is complete. Estimated charge into the battery is still labelled as an estimate — Growatt does not measure “charge from string N”.</p>
<p>
...
<p>
<b>Day</b> sits next to Reload charts. It chooses which London day the charts and the energy lines on the cards show. The stepper or the calendar moves to an earlier day; <b>Today</b> comes back to the current day. A future day is not offered. The big kW figures on the cards stay live (what the inverter is doing now). The kWh line under them, and both charts, follow the day you picked. On a past day the teal Now line is hidden, and the totals are the stored samples only — they are not stretched up to the current clock.</p>
<p>
